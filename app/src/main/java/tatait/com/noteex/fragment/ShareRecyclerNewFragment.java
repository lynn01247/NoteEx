 ...
 * 描述:
 */
public class ShareRecyclerNewFragment extends BaseFragment implements BGAOnItemChildClickListener, BGAOnRVItemClickListener, BGAOnRVItemLongClickListener,
    ...
    protected void initView(Bundle savedInstanceState) {
        ...
        CommonUtil.itemClickNewHandler = mHandler;
    }
...
//        mDataRv.addItemDecoration(new Divider(mApp));
        ...
        pageSize = (int) SharedPreferencesUtils.getParam(mApp, CommonUtil.PAGESIZE_NEW_SHARED, 10);
        order = (int) SharedPreferencesUtils.getParam(mApp, CommonUtil.ORDER_NEW_SHARED, 1);
    ...
    public boolean onRVItemLongClick(ViewGroup viewGroup, View itemView, int position) {
        if (!(Boolean) SharedPreferencesUtils.getParam(mApp, CommonUtil.ISLOGIN, false)) {
            ...
            CommonUtil.itemClickNewHandler = null;
    ...
    public void refreshGetData(final Boolean isRefreshed) {
        ...
        showLoadingDialog();
        if (!(Boolean) SharedPreferencesUtils.getParam(mApp, CommonUtil.ISLOGIN, false)) {
            getDataUrl = HttpRoute.URL_HEAD + HttpRoute.URL_GET_NOTE_TYPE + "?uid=" + uid + "&token=" + token + "&order=" + order + "&pageIndex=" +
                    mMorePageNumber + "&pageSize=" + pageSize + "&type=new_shared";
        } else {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.sore_rl1:
                sore_create_iv.setVisibility(View.VISIBLE);
                sore_update_iv.setVisibility(View.GONE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.ORDER_NEW_SHARED, 1);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.sore_rl2:
                sore_create_iv.setVisibility(View.GONE);
                sore_update_iv.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.ORDER_NEW_SHARED, 2);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.see_rl:
                ...
                see_show_iv3.setVisibility(View.GONE);
                see_show_iv.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.PAGESIZE_NEW_SHARED, 10);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.see_rl2:
                ...
                see_show_iv3.setVisibility(View.GONE);
                see_show_iv2.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.PAGESIZE_NEW_SHARED, 20);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.see_rl3:
                ...
                see_show_iv.setVisibility(View.GONE);
                see_show_iv3.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.PAGESIZE_NEW_SHARED, 40);
                dialog.dismiss();
                refreshGetData(true);
                break;
        }
    ...
    private void collectNoteById(GsonNoteModel.NoteModel item) {
        if (item != null) {
            int nid = item.getNid();
            CommonUtil.collectNoteNewHandler = mHandler;
            showLoadingDialog();
                ...
                public void onResponse(GsonNoteTipModel gsonModel) {
                    ...
                    if (gsonModel != null) {
                        ...
                        if (Integer.parseInt(gsonModel.getCode()) == 1) {
                            if (CommonUtil.collectNoteNewHandler != null) {
                                Message message = new Message();
                                message.what = CommonUtil.collectNoteNewCode;
                                message.obj = gsonModel.getInfo();
                                CommonUtil.collectNoteNewHandler.sendMessage(message);
                            }
                        } else {
                            ToastUtil.show(gsonModel.getInfo());
                        }
    ...
    private void delNoteCollectById(GsonNoteModel.NoteModel item) {
        if (item != null) {
            int nid = item.getNid();
            CommonUtil.delCollectNoteNewHandler = mHandler;
            showLoadingDialog();
            ...
            OkHttpClientManager.getAsyn(delDataUrl, new OkHttpClientManager.ResultCallback<GsonNoteTipModel>() {
                ...
                public void onResponse(GsonNoteTipModel gsonModel) {
                    ...
                    if (gsonModel != null) {
                        ToastUtil.show(gsonModel.getInfo());
                        if (Integer.parseInt(gsonModel.getCode()) == 1) {
                            if (CommonUtil.delCollectNoteNewHandler != null) {
                                Message message = new Message();
                                message.what = CommonUtil.delCollectNoteNewCode;
                                message.obj = gsonModel.getInfo();
                                CommonUtil.delCollectNoteNewHandler.sendMessage(message);
                            }
                        } else {
                            ToastUtil.show(gsonModel.getInfo());
                        }
    ...
    private void PraiseNoteById(GsonNoteModel.NoteModel item) {
        if (item != null) {
            int nid = item.getNid();
            CommonUtil.praisedNoteNewHandler = mHandler;
            showLoadingDialog();
            ...
            OkHttpClientManager.postAsyn(delDataUrl, new OkHttpClientManager.ResultCallback<GsonNoteTipModel>() {
                ...
                public void onResponse(GsonNoteTipModel gsonModel) {
                    ...
                    if (gsonModel != null) {
                        ToastUtil.show(gsonModel.getInfo());
                        if (Integer.parseInt(gsonModel.getCode()) == 1) {
                            if (CommonUtil.praisedNoteNewHandler != null) {
                                Message message = new Message();
                                message.what = CommonUtil.praisedNoteNewCode;
                                message.obj = gsonModel.getInfo();
                                CommonUtil.praisedNoteNewHandler.sendMessage(message);
                            }
                        } else {
                            ToastUtil.show(gsonModel.getInfo());
                        }
    ...
    public Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case CommonUtil.itemClickNewCode:
                    startActivity(new Intent(getContext(), NewNoteActivity.class));
                    break;
                case CommonUtil.dialogCode:
                    CommonUtil.itemClickNewHandler = mHandler;
                    break;
                case CommonUtil.collectNoteNewCode:
                    refreshGetData(true);
                    break;
                case CommonUtil.delCollectNoteNewCode:
                    refreshGetData(true);
                    break;
                case CommonUtil.praisedNoteNewCode:
                    refreshGetData(true);
                    break;
            }
        }
