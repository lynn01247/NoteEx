 ...
 * 描述:
 */
public class ShareRecyclerGemFragment extends BaseFragment implements BGAOnItemChildClickListener, BGAOnRVItemClickListener, BGAOnRVItemLongClickListener,
...
//        mDataRv.addItemDecoration(new Divider(mApp));
        ...
        pageSize = (int) SharedPreferencesUtils.getParam(mApp, CommonUtil.PAGESIZE_GEM_SHARED, 10);
        order = (int) SharedPreferencesUtils.getParam(mApp, CommonUtil.ORDER_GEM_SHARED, 1);
    ...
    public boolean onRVItemLongClick(ViewGroup viewGroup, View itemView, int position) {
        if (!(Boolean) SharedPreferencesUtils.getParam(mApp, CommonUtil.ISLOGIN, false)) {
            ...
            CommonUtil.itemClickGemHandler = null;
    ...
    public void refreshGetData(final Boolean isRefreshed) {
        ...
        if (!(Boolean) SharedPreferencesUtils.getParam(mApp, CommonUtil.ISLOGIN, false)) {
            getDataUrl = HttpRoute.URL_HEAD + HttpRoute.URL_GET_NOTE_TYPE + "?order=" + order + "&pageIndex=" +
                    mMorePageNumber + "&pageSize=" + pageSize + "&type=gem_shared";
        } else {
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.sore_rl1:
                sore_create_iv.setVisibility(View.VISIBLE);
                sore_update_iv.setVisibility(View.GONE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.ORDER_GEM_SHARED, 1);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.sore_rl2:
                sore_create_iv.setVisibility(View.GONE);
                sore_update_iv.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.ORDER_GEM_SHARED, 2);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.see_rl:
                ...
                see_show_iv3.setVisibility(View.GONE);
                see_show_iv.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.PAGESIZE_GEM_SHARED, 10);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.see_rl2:
                ...
                see_show_iv3.setVisibility(View.GONE);
                see_show_iv2.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.PAGESIZE_GEM_SHARED, 20);
                dialog.dismiss();
                refreshGetData(true);
                break;
            case R.id.see_rl3:
                ...
                see_show_iv.setVisibility(View.GONE);
                see_show_iv3.setVisibility(View.VISIBLE);
                SharedPreferencesUtils.setParam(mApp, CommonUtil.PAGESIZE_GEM_SHARED, 40);
                dialog.dismiss();
                refreshGetData(true);
                break;
        }
    ...
    private void collectNoteById(GsonNoteModel.NoteModel item) {
        if (item != null) {
            int nid = item.getNid();
            CommonUtil.collectNoteGemHandler = mHandler;
            showLoadingDialog();
                ...
                public void onResponse(GsonNoteTipModel gsonModel) {
                    ...
                    if (gsonModel != null) {
                        ...
                        if (Integer.parseInt(gsonModel.getCode()) == 1) {
                            if (CommonUtil.collectNoteGemHandler != null) {
                                Message message = new Message();
                                message.what = CommonUtil.collectNoteGemCode;
                                message.obj = gsonModel.getInfo();
                                CommonUtil.collectNoteGemHandler.sendMessage(message);
                            }
                        } else {
                            ToastUtil.show(gsonModel.getInfo());
                        }
    ...
    private void delNoteCollectById(GsonNoteModel.NoteModel item) {
        if (item != null) {
            int nid = item.getNid();
            CommonUtil.delCollectNoteGemHandler = mHandler;
            showLoadingDialog();
            ...
            OkHttpClientManager.getAsyn(delDataUrl, new OkHttpClientManager.ResultCallback<GsonNoteTipModel>() {
                ...
                public void onResponse(GsonNoteTipModel gsonModel) {
                    ...
                    if (gsonModel != null) {
                        ToastUtil.show(gsonModel.getInfo());
                        if (Integer.parseInt(gsonModel.getCode()) == 1) {
                            if (CommonUtil.delCollectNoteGemHandler != null) {
                                Message message = new Message();
                                message.what = CommonUtil.delCollectNoteGemCode;
                                message.obj = gsonModel.getInfo();
                                CommonUtil.delCollectNoteGemHandler.sendMessage(message);
                            }
                        } else {
                            ToastUtil.show(gsonModel.getInfo());
                        }
    ...
    private void PraiseNoteById(GsonNoteModel.NoteModel item) {
        if (item != null) {
            int nid = item.getNid();
            CommonUtil.praisedNoteGemHandler = mHandler;
            showLoadingDialog();
            ...
            OkHttpClientManager.postAsyn(delDataUrl, new OkHttpClientManager.ResultCallback<GsonNoteTipModel>() {
                ...
                public void onResponse(GsonNoteTipModel gsonModel) {
                    ...
                    if (gsonModel != null) {
                        ToastUtil.show(gsonModel.getInfo());
                        if (Integer.parseInt(gsonModel.getCode()) == 1) {
                            if (CommonUtil.praisedNoteGemHandler != null) {
                                Message message = new Message();
                                message.what = CommonUtil.praisedNoteGemCode;
                                message.obj = gsonModel.getInfo();
                                CommonUtil.praisedNoteGemHandler.sendMessage(message);
                            }
                        } else {
                            ToastUtil.show(gsonModel.getInfo());
                        }
    ...
    public Handler mHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                case CommonUtil.itemClickGemCode:
                    startActivity(new Intent(getContext(), NewNoteActivity.class));
                    break;
                case CommonUtil.dialogCode:
                    CommonUtil.itemClickGemHandler = mHandler;
                    break;
                case CommonUtil.collectNoteGemCode:
                    refreshGetData(true);
                    break;
                case CommonUtil.delCollectNoteGemCode:
                    refreshGetData(true);
                    break;
                case CommonUtil.praisedNoteGemCode:
                    refreshGetData(true);
                    break;
            }
        }
